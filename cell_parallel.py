        ...
        with tf.variable_scope(scope or type(self).__name__, reuse=tf.AUTO_REUSE):
            with tf.device('/gpu:0'):
             with tf.variable_scope("Prior"):
                ...
                prior_zeta_hidden = fc_layer(y_s_stack, self.z_dim, activation=tf.nn.softplus, scope="zeta_hidden")
                prior_z_hidden = fc_layer(y_t_stack, self.z_dim, activation=tf.nn.softplus, scope="z_hidden")
            ...
            with tf.device('/gpu:1'):
             with tf.variable_scope("Encoder"):
                ...
                h_inter_enc = fc_layer(h_t, self.n_h, scope="intermidiate")
                ...
                for i in range(self.n):
                    ...
                    # print "Debug intermediate size", intermediate.get_shape(), h_inter.get_shape()
                    # intermediate = tf.add_n(tf.multiply(tf.gather_nd(self.adj, (i, u)), tf.gather(y, u)), tf.multiply(tf.gather_nd(self.adj, (i,v)), tf.gather(y, v)))
                    first = tf.concat([y_current[i], intermediate, h_inter_enc], axis=1)
                    second = tf.concat([tf.zeros([1, 2 * self.d]), h_inter_enc], axis=1)
                    m_t = tf.convert_to_tensor([[type_m * t]])
                    y_t.append(tf.concat([tf.add(first, second), tf.cast(m_t, tf.float32)], axis = 1))

                y_s_enc_stack = tf.reshape(tf.stack(y_s), [self.n,-1])
                y_t_enc_stack = tf.reshape(tf.stack(y_t), [self.n, -1])
                # print "Debug Y", y_s_stack.shape, y_t_stack.shape

                enc_zeta_hidden = fc_layer( y_s_enc_stack, self.z_dim,  scope="zeta_hidden" )
                enc_z_hidden = fc_layer( y_t_enc_stack, self.z_dim,  scope="z_hidden" )
