	...
	Args:
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            with tf.variable_scope("Decoder"):
                ...
                r_hidden = fc_layer(tf.concat([h_t, tf.transpose(tf.gather(zeta, u)), tf.transpose(tf.gather(zeta, v))], axis=1), self.n_c, activation=tf.nn.softplus, scope="R_hidden" )
                r = tf.cast(1 - type_m, tf.float32) * self.r_old + tf.cast(type_m, tf.float32) * r_hidden
                P = tf.multiply(tf.subtract(tf.ones([self.n, self.n]), self.adj), (self.alpha * tf.matmul(tf.matmul(tf.transpose(c), B), c) + (1 - self.alpha) * r))
