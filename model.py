...
class MPPModel():
    def __init__(self, args, sample=False):
        ...
        self.input_data = tf.placeholder(dtype=tf.float32, shape=[args.batch_size, args.seq_length, 1], name='input_data')
        self.target_data = tf.placeholder(dtype=tf.float32, shape=[args.batch_size, args.seq_length, 1], name='target_data')
        #TODO
        self.adj = tf.placeholder(dtype=tf.int32, shape=[args.n, args.n], name='adj')
        
        [self.initial_state_s, self.initial_state_c] = cell.zero_state(batch_size=args.batch_size, dtype=tf.float32)
